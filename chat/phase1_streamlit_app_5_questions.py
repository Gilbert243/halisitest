...
if "current_group" not in st.session_state:          # index du groupe de questions en cours
...
MAX_QUESTIONS = 5  # Seulement 5 questions composites

# --------------------------------------------------
# TOPICS (liste complète inchangée)
# --------------------------------------------------
DEMOGRAPHIC_TOPICS = [
...
# --------------------------------------------------
# GROUPES DE QUESTIONS COMPOSITES
# --------------------------------------------------
QUESTION_GROUPS = [
    {
        "name": "Contexte",
...
# --------------------------------------------------
# NORMALIZATION HELPERS (inchangés)
# --------------------------------------------------
...
# --------------------------------------------------
# CATEGORIZATION (inchangée, mais on va l'appeler pour chaque topic)
# --------------------------------------------------
...
# --------------------------------------------------
# GÉNÉRATION DE LA PROCHAINE QUESTION (GROUPE)
# --------------------------------------------------
def generate_next_group_question():
    """Retourne la question du groupe courant et le groupe suivant (index)."""
    ...
    # Adaptation du contexte (self/child) pour la démographie
    if group["name"] == "Démographie" and "product_context" in st.session_state.extracted_info:
        ctx = st.session_state.extracted_info["product_context"].get("value")
        if ctx == "child":
            # On peut modifier légèrement la question pour un enfant
            base_question = "What is your child's age and gender? (Your location also helps for climate recommendations)"

    # On peut éventuellement utiliser LLM pour rendre la question plus naturelle,
    # mais on garde la question prédéfinie pour l'instant.
    return base_question, st.session_state.current_group

# --------------------------------------------------
# PROFIL GENERATION (inchangée)
# --------------------------------------------------
def generate_clean_profile():
    """Generate a profile with correctly categorized values."""
    profile_data = {}
    meaningful_count = 0
...
if not st.session_state.messages:
    # On initialise avec la première question du premier groupe
    first_group = QUESTION_GROUPS[0]
    st.session_state.messages.append({
        "role": "assistant",
        "content": first_group["question"]
    })
    st.session_state.current_group = 0
...
st.markdown("*Quick, comprehensive hair profiling in just 5 questions*")
...
if not st.session_state.conversation_complete:
    ...
    if user_input:
        ...
        # Extraire les informations pour tous les topics du groupe courant
        if st.session_state.current_group < len(QUESTION_GROUPS):
            group = QUESTION_GROUPS[st.session_state.current_group]
            for topic in group["topics"]:
                ...
                # Stocker aussi dans les dicts démographiques si nécessaire
        ...
        if should_end:
            # Traitement spécial pour la grossesse (on peut le faire après coup)
            # Si le genre est Male, marquer pregnancy_status comme Not applicable si absent
            ...
            summary = st.session_state.profile_data.get("summary", "Profile ready")
...
We can now provide personalized recommendations."""
            st.session_state.messages.append({"role": "assistant", "content": complete_msg})
            st.rerun()

        else:
            # Poser la question du groupe suivant
...
# --------------------------------------------------
st.markdown("---")
st.caption("Halisi Cosmetics • Smart Hair Profiler • v3.0 – 5 questions only")