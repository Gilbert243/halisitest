...
# --------------------------------------------------
# PROFILE GENERATION (with image storage and analysis)
# --------------------------------------------------
def generate_clean_profile():
    """Generate a profile with correctly categorized values, captured image, and analysis results."""
    profile_data = {}
    meaningful_count = 0
    ...
    # Add image analysis result to summary
    if st.session_state.analysis_results:
        hair_type = st.session_state.analysis_results.get('hair_type', 'unknown')
...
# --------------------------------------------------
# PHOTO CAPTURE + ANALYSIS (after questions)
# --------------------------------------------------
if st.session_state.conversation_complete and not st.session_state.image_analyzed:
    ...
    st.write("This helps us analyze your hair texture and condition for better recommendations.")
    ...
    if not azure_api_key or not azure_endpoint:
        st.warning("⚠️ Azure credentials not found. Image analysis will be skipped.")
        # Allow skipping directly
        ...
        if camera_image is not None:
            ...
            try:
                with st.spinner("Analyzing your hair with AI..."):
                    ...
                    # Display preview
                    hair_type = result['hair_type'].get('hair_type', 'unknown')
                    st.success(f"Analysis complete! Detected hair type: {hair_type}")
                ...
                st.session_state.image_captured = True
                st.session_state.image_analyzed = True
                st.warning("Continuing without image analysis.")
            finally:
                # Clean up temp file
                os.unlink(temp_path)
...
# --------------------------------------------------
st.markdown("---")
st.caption("Halisi Cosmetics • Smart Hair Profiler • v4.0 – with AI image analysis")